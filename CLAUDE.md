...
- **`mobile/`** (`com.meatsack.motivator.mobile`) — Phone companion. `MainActivity` hosts a Compose `NavGraph` with bottom nav for Library and Settings. `SettingsRepository` backs a DataStore. `LibraryViewModel` reads from the shared Room DB; `LibraryScreen` has a Sync-to-Watch button that calls `PhoneSyncSender` (serializes active messages with fewer than 3 downvotes and writes a DataItem at `/messages`).
...
- **`wear/trigger/`** (v2) — `TriggerScheduler` + `BehindPaceWorker` (daily at configurable hour, default noon) + `EndOfDayWorker` (daily at configurable hour). Both workers read step count from a SharedPreferences hand-off updated by `HealthTracker`, then select a message for the appropriate trigger type via `MessageRepository`. Both self-reschedule for tomorrow at the top of `doWork` so a single early-return / OS-kill can't break the daily chain.
